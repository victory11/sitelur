...
const roleBadge: Record<string, { label: string; color: string; icon: string }> = {
  admin: { label: "Admin", color: "bg-purple-500", icon: "⚡" },
  notulis: { label: "Notulis", color: "bg-blue-500", icon: "📝" },
    ...
    <>
      ...
      >
        <div className="flex items-center gap-3 px-4 py-5 border-b border-white/10">
          <div className="w-10 h-10 bg-[#1A6EB5] rounded-xl flex items-center justify-center font-bold text-lg shrink-0">🏥</div>
          {!collapsed && (
          ...
          <button onClick={() => { setCollapsed(!collapsed); setMobileOpen(false); }} className="ml-auto text-gray-400 hover:text-white hidden lg:block">
          ...
          {visibleNav.map((item) => (
            <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all
                ${isActive(item.href) ? "bg-[#1A6EB5] text-white shadow-lg shadow-[#1A6EB5]/25" : "text-gray-300 hover:bg-white/10 hover:text-white"}
              `}
